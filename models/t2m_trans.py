...
class Text2Motion_Transformer_New(nn.Module):
    ...
    def __init__(self, 
        ...
        self.n_head = n_head
        self.trans_base = CrossCondTransBase_New(vqvae, num_vq, embed_dim, modernbert.config.hidden_size, block_size, num_layers, num_local_layer, n_head, drop_out_rate, fc_rate)
        self.trans_head = CrossCondTransHead_New(num_vq, modernbert.config.vocab_size, embed_dim, block_size, num_layers, n_head, drop_out_rate, fc_rate)
        self.block_size = block_size
        self.num_vq = num_vq
...
class Text2Motion_Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.n_head = n_head
        self.trans_base = CrossCondTransBase(vqvae, num_vq, embed_dim, clip_dim, block_size, num_layers,
                                             num_local_layer, n_head, drop_out_rate, fc_rate)
    ...
    def sample(self, clip_feature, word_emb, m_length=None, if_test=False, rand_pos=True, CFG=-1, token_cond=None,
               max_steps=10):
        max_length = 49
        batch_size = clip_feature.shape[0]
        mask_id = self.num_vq + 2
        ...
        for step in range(max_steps):
            timestep = torch.clip(step / (sample_max_steps), max=1)
            if len(m_tokens_len) == 1 and step > 0 and torch.clip(step - 1 / (sample_max_steps),
                                                                  max=1).cpu().item() == timestep:
                break
            rand_mask_prob = cosine_schedule(timestep)  # timestep #
            num_token_masked = (rand_mask_prob * m_tokens_len).long().clip(min=1)
...
class CrossCondTransBase_New(nn.Module):
...
class CrossCondTransHead_New(nn.Module):
...
class CrossCondTransBase(nn.Module):
    ...
    def __init__(self,
        ...
        self.drop = nn.Dropout(drop_out_rate)
        # transformer block
        self.blocks = nn.Sequential(*[Block(embed_dim, block_size, n_head, drop_out_rate, fc_rate) for _ in
        ...
        if num_local_layer > 0:
            ...
            self.cross_att = nn.Sequential(
                *[Block_crossatt(embed_dim, block_size, n_head, drop_out_rate, fc_rate) for _ in
    ...
    def forward(self, idx, clip_feature, src_mask, word_emb):
        if len(idx) == 0:
            ...
            token_embeddings = torch.empty((*idx.shape, self.vqvae.vqvae.code_dim), device=idx.device)
            token_embeddings[not_learn_idx] = self.vqvae.vqvae.quantizer.dequantize(idx[not_learn_idx]).requires_grad_(
                False)
            token_embeddings[learn_idx] = self.learn_tok_emb(idx[learn_idx] - self.vqvae.vqvae.num_code)
